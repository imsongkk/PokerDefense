...
namespace Moments.Encoder
{
	public class GifEncoder
	{
		...
		protected void AnalyzePixels()
		{
			...
			if(useAlpha)
			{
				int tR = (int)(transparancy.r * 255f);
				int tG = (int)(transparancy.g * 255f);
				int tB = (int)(transparancy.b * 255f);
				transIndex = nq.Map(tR,tG,tB);
			}
		}

		protected int FindClosest(Color c) 
		{
			if (m_ColorTab == null) return -1;
			int r = 255;
			int g = 0;
			int b = 0;
			int minpos = 0;
			int dmin = 256 * 256 * 256;
			int len = m_ColorTab.Length;
			for (int i = 0; i < len;) 
			{
				int dr = r - (m_ColorTab[i++] & 0xff);
				int dg = g - (m_ColorTab[i++] & 0xff);
				int db = b - (m_ColorTab[i] & 0xff);
				int d = dr * dr + dg * dg + db * db;
				int index = i / 3;
				if (m_UsedEntry[index] && (d < dmin)) 
				{
					dmin = d;
					minpos = index;
				}
